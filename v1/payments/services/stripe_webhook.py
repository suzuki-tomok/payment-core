...
class StripeWebhookService:
    ...
    def handle_checkout_completed(data: object) -> None:
        ...
        外部API (Stripe) の呼び出しは DB トランザクションの外で行い、
        DB 接続を長時間保持しないようにする。
        """
        ...
        session = None
        if checkout.type in ("credit", "custom"):
        ...
        with transaction.atomic():
            ...
            if checkout.type == "credit":
                StripeWebhookService._create_credit_history(checkout, session)  # noqa: SLF001
            elif checkout.type == "custom":
                StripeWebhookService._create_invoice_history(checkout, session)  # noqa: SLF001
    ...
    @staticmethod
    def _create_credit_history(checkout: CheckoutSessionStatus, session: object) -> None:
        """クレジット購入の CreditStatus を作成.

        session は呼び出し元が事前取得した stripe.checkout.Session。
        リトライで再実行された場合に備え、既存チェックで冪等性を担保する。
        """
        payment_intent_id = str(session.payment_intent)  # type: ignore[attr-defined]
        price_id = session.line_items.data[0].price.id  # type: ignore[attr-defined]
        cid = checkout.stripe_customer.stripe_customer_id

        if CreditStatus.objects.filter(stripe_payment_id=payment_intent_id).exists():
            logger.info("CreditStatus already exists, skipping: pid=%s cus=%s", payment_intent_id, cid)
            return
    ...
    @staticmethod
    def _create_invoice_history(checkout: CheckoutSessionStatus, session: object) -> None:
        ...
        """
        payment_intent_id = str(session.payment_intent)  # type: ignore[attr-defined]
        line_item = session.line_items.data[0]  # type: ignore[attr-defined]
        description = line_item.description or "カスタム支払い"
        amount = line_item.amount_total
        cid = checkout.stripe_customer.stripe_customer_id

        if InvoiceStatus.objects.filter(stripe_payment_id=payment_intent_id).exists():
            logger.info("InvoiceStatus already exists, skipping: pid=%s cus=%s", payment_intent_id, cid)
            return
    ...
    def handle_subscription_created(data: object) -> None:
        ...
        webhook payload (data) に Subscription オブジェクトが全量含まれるため、
        Stripe への retrieve は不要。リトライで再実行された場合に備え、
        既存チェックで冪等性を担保する。
        """
        stripe_customer_id = data["customer"]  # type: ignore[index]
        stripe_subscription_id = data["id"]  # type: ignore[index]

        if SubscriptionStatus.objects.filter(stripe_subscription_id=stripe_subscription_id).exists():
            logger.info("SubStatus already exists, skipping: sub=%s cus=%s", stripe_subscription_id, stripe_customer_id)
            return

        item = data["items"]["data"][0]  # type: ignore[index]
        ...
        SubscriptionStatus.objects.create(
            ...
            status=data["status"],  # type: ignore[index]
    ...
    def handle_subscription_updated(data: object) -> None:
        ...
        プラン変更・月次更新の両方に対応。
        webhook payload (data) に Subscription オブジェクトが全量含まれるため、
        Stripe への retrieve は不要。price_id から SubscriptionPlan を特定し、
        プランも更新する。
        """
        stripe_subscription_id = data["id"]  # type: ignore[index]
        stripe_customer_id = data["customer"]  # type: ignore[index]
