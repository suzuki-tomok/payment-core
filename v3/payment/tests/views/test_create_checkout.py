...
@pytest.mark.django_db
def test_duplicate_order_returns_409(client, mock_stripe_client):
    """同 order_id 既存 → 409 + order_id を含む body."""
    PaymentFactory(order_id="ord-dup")
    body = _valid_body()
    body["order_id"] = "ord-dup"

    response = _post_checkout(client, body)

    assert response.status_code == 409
    assert response.json() == {"error": "duplicate_order", "order_id": "ord-dup"}
